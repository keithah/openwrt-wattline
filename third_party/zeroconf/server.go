...
func (s *Server) waitOrShutdown(delay time.Duration) bool {
...
func (s *Server) probe() {
	...
	for i := 0; i < multicastRepetitions; i++ {
		...
		if !s.waitOrShutdown(time.Duration(randomizer.Intn(250)) * time.Millisecond) {
			return
		}
	}
